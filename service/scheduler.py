...
def extract(fl,path):
    ...
    try:
        ...
        try:
            ...
            app.logger.error('Scheduler - Extraction could not be done for ' + fl + ' : "{x}"'.format(x=e))
            return False
            ...
            if len(os.listdir(pubstoredir + '/' + dir + '/pending')):
                for transact in os.listdir(pubstoredir + '/' + dir + '/pending'):
                    ...
                    try:
                        ...
                        app.logger.error("Scheduler - failed to delete pending entry: '{x}'".format(x=e))
            else:
...
def processftp():
    try:
        ...
        app.logger.error('Scheduler - failed scheduled process for FTP temp directories: "{x}"'.format(x=e))
...
def checkunrouted():
    ...
    try:
        ...
        app.logger.error("Scheduler - Failed scheduled check for unrouted notifications: '{x}'".format(x=e))
...
def monthly_reporting():
    ...
    try:
        ...
        app.logger.error("Scheduler - Failed scheduled reporting job: '{x}'".format(x=e))
...
def delete_old_routed():
    ...
    try:
        ...
        app.logger.error("Scheduler - Failed monthly routed index deletion: '{x}'".format(x=e))
